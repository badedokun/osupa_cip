...
async function dataCompare(ufirstName, umiddleName, ulastName, IDParseJSON) {
    ...
    await db
        ...
        snapshot.docs.forEach((doc) => {
            const name_fuzz_ratio = fuzz.ratio(ufirstName + umiddleName + ulastName, IDParseJSON["name"]);
            const address_fuzz_ratio = fuzz.ratio(doc.data().address, IDParseJSON["address"]);
            //dob_fuzz_ratio
        ...
        try {
            if (data["SDNTYPE"] == "individual") {
                ...
                if (name[0] != null && name[1] != null) {
                    doesExist = await ofacExistenceCheck(name[0].trim(), name[1].trim());
                }
            }
        }
        catch (err) {
...
exports.KYCVerification = functions.https.onCall(async (req, context) => {
    ...
    const faceCompareResponse = await faceCompare(idImage, selfie);
    ...
    let dataCompareResponse = await dataCompare(userFirst, userMiddle, userLast, IDParseResponse);
