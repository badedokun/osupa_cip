...
async function dataCompare(ufirstName,umiddleName,ulastName,IDParseJSON){
        ...
        await db.collection('users').where('firstName','==',ufirstName).where('middleName','==',umiddleName).where('lastName','==',ulastName).get().then((snapshot) => {
                snapshot.docs.forEach(doc => {
                        ...
                        address_fuzz_ratio=fuzz.ratio(doc.data().address , IDParseJSON["address"]);
                        //dob_fuzz_ratio
 ...
 */
 exports.KYCVerification = functions.https.onRequest(async (req, res) => {
         ...
         var idImage = req.query.IDimageLink;
         var selfie = req.query.userSelfie;
         var userFirst = req.query.firstName;
         var userMiddle= req.query.middleName;
         var userLast = req.query.lastName;
         var bankNumber = req.query.accountNumber;
         var KYCScore=0;
         //setPixlabApiKey()
         await plaidInit();
        var IDParseResponse= await IDParsing(idImage,'malaysia');//'united states'
        var faceCompareResponse= await faceCompare(idImage,selfie);
        ...
        var dataCompareResponse= await dataCompare(userFirst,userMiddle,userLast,IDParseResponse);
        if(dataCompareResponse != 20){
                failureStatus.push("Data comparison failed");
        }
        KYCScore=KYCScore+dataCompareResponse;

        var sdnResponse = await ofacScreening(userFirst.toString(),userLast.toString());
        if(sdnResponse != 20){
                failureStatus.push("Sdn list lookup failed");
        }
        KYCScore =KYCScore+sdnResponse;
        
        var PEPResponse= await PEPScreening(userFirst.toString(),userLast.toString());
        if(PEPResponse != 20){
        ...
        var bankResponse= await bankAccount(bankNumber,"access-sandbox-8352b978-6800-4aec-bf32-1aa927f784a3");
        if(bankResponse != 20){
                failureStatus.push("Bank verification failed");
        }
        KYCScore =KYCScore+ bankResponse;
        res.send(KYCScore.toString()+" Failure status:"+failureStatus);
